...
//likePushNotification('16508420492', 'd8143166a6e3d0f1abc20ab452c6becd7f56564c2320c8389fa45cc358e25eaa37c4e0199f31fbd6595eaeca68fe23590f0dd8

require "../stream/1.0/api/DEPENDENCIES/twilio/Services/Twilio.php";
	...
	if($commenter_phone !== $uploader_phone)
	{
		...
		while($uploader_information_row = mysql_fetch_array($uploader_information_result))
		{
			...
			if($uploader_token !== '')
			{
				...
				$is_comment = 1;
				$url = 'http://75.101.134.112/stream/1.0/api/push_notification.php?token=' . $uploader_token . '&message=' . urlencode($message) . '&stream_id=' . $small_stream_id . '&picture_id=' . $small_picture_id . '&badge_count=' . $badge_count . '&is_comment=1';
			  	$ch = curl_init($url);
	...
	while($users_in_stream_row = mysql_fetch_array($users_in_stream_result))
	{
		...
		while($user_information_row = mysql_fetch_array($user_information_result))
		{
			...
			if($user_token !== '')
			{
				...
				$small_stream_id = substr($stream_id, 0, 5);
				$badge_count = $uploader_information_row['BadgeCount'] +1;
				$url = 'http://75.101.134.112/stream/1.0/api/push_notification.php?token=' . $user_token . '&message=' . urlencode($message) . '&stream_id=' . $small_stream_id . '&picture_id=' . $small_picture_id . '&badge_count=' . $badge_count . '&is_comment=1'; 
			  	$ch = curl_init($url);
			  	$response = curl_exec($ch);
			  	curl_close($ch);
