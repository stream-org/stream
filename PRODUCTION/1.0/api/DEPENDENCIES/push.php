	...
	while($users_in_stream_row = mysql_fetch_array($users_in_stream_result))
	{
		...
		while($user_information_row = mysql_fetch_array($user_information_result))
		{
			...
			if($user_token !== '')
			{
				...
				$url = 'http://75.101.134.112/stream/1.0/api/push_notification.php?token=' . $user_token . '&message=' . urlencode($message). '&picture_id=' .$picture_id.  '&badge_count='.$badge_count; 
			  	$ch = curl_init($url);
			  	$response = curl_exec($ch);
			  	curl_close($ch);
...
function uploadPicturePushNotification($uploader_phone, $stream_id, $picture_id)
{
	...
	for($iPhone_user_token_index = 0; $iPhone_user_token_index < count($invited_iPhone_users); $iPhone_user_token_index++)
	{
		...
		$url = 'http://75.101.134.112/stream/1.0/api/push_notification.php?token=' . $current_token . '&message=' . urlencode($message). '&picture_id='.$picture_id. '&badge_count='.$badge_count; 
	  	$ch = curl_init($url);
	  	$response = curl_exec($ch);
	  	curl_close($ch);
